...
### 6. Get-RDPCurrentSessions: Removed -SessionID Parameter

**Design Decision:** Removed `-SessionID` parameter from Get-RDPCurrentSessions in favor of standard PowerShell filtering.
...
```powershell
# OLD (removed):
Get-RDPCurrentSessions -SessionID 3 -ShowProcesses

# NEW (recommended):
Get-RDPCurrentSessions | Where-Object { $_.ID -eq 3 }
Get-RDPCurrentSessions -ShowProcesses  # Shows processes for ALL sessions
```
...
**Removed:**
- Get-RDPCurrentSessions `-SessionID` parameter (use PowerShell filtering instead)
...
- Get-RDPCurrentSessions simplicity (standard PowerShell filtering patterns)
